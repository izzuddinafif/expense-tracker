...
require_environment() {
    ...
    for name in LEDGERLY_SIGNING_STORE_FILE STORE_PASSWORD KEY_ALIAS KEY_PASSWORD; do
        if [[ -z "${!name:-}" ]]; then
            missing+=("${name}")
        fi
    done
    ...
    if ((${#missing[@]} > 0)); then
        ...
        printf 'Set all four explicitly; this script will not fall back to Android Debug signing.\n' >&2
        exit 1
    fi
}
